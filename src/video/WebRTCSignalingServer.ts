...
export class WebRTCSignalingServer extends EventEmitter {
  ...
  private participantRoomMap: Map<string, string> = new Map();
  private roomCodeMap: Map<string, string> = new Map(); // roomCode -> roomId
  private connectionHandlers: Map<string, (message: SignalingMessage) => void> = new Map();
   ...
   */
  async createRoom(request: CreateRoomRequest): Promise<VideoConferenceRoom> {
    ...
    const compressionSettings = new VideoCompressionConfig(
      request.compressionSettings?.baseQuality || 'medium',
      request.compressionSettings?.adaptiveQuality ?? true,
      request.compressionSettings?.quantumCompressionLevel || 5,
      request.compressionSettings?.bandwidthThreshold || 2
    );
   ...
   */
  async joinRoom(request: JoinRoomRequest): Promise<{ room: VideoConferenceRoom; participant: Participant }> {
   ...
   */
  private async optimizeBandwidthDistribution(roomId: string): Promise<BandwidthOptimizationResult> {
    const room = this.rooms.get(roomId);
    if (!room) {
    ...
    const result: BandwidthOptimizationResult = {
   ...
   */
  private forwardSignalingMessage(message: SignalingMessage): void {
    const room = this.rooms.get(message.roomId);
    if (!room) return;
   ...
   */
  private async handleJoinRoomMessage(message: SignalingMessage): Promise<void> {
    const joinRequest: JoinRoomRequest = message.data;
    try {
