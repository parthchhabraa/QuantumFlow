...
exports.QuantumPhaseAssigner = void 0;
 ...
 */
class QuantumPhaseAssigner {
    constructor(strategy = 'entropy-based', adaptiveThreshold = 0.5) {
     ...
     */
    calculateEntropyBasedPhase(chunk) {
        if (chunk.length === 0)
            return 0;
     ...
     */
    calculateFrequencyBasedPhase(chunk) {
        if (chunk.length === 0)
            return 0;
        const frequencies = new Array(256).fill(0);
        for (const byte of chunk) {
        ...
        const basePhase = QuantumMath_1.QuantumMath.calculateQuantumPhase(maxFreqIndex);
     ...
     */
    calculatePatternBasedPhase(chunk) {
        if (chunk.length < 2)
            return this.calculateEntropyBasedPhase(chunk);
        let patternScore = 0;
        let transitions = 0;
     ...
     */
    calculateAdaptivePhase(chunk, context) {
        if (chunk.length === 0)
            return 0;
        ...
        const weightedPhase = (entropyPhase * weights[0]) +
     ...
     */
    calculateCorrelationBasedPhase(chunk, context) {
     ...
     */
    calculateUniformity(chunk) {
        const frequencies = new Array(256).fill(0);
        for (const byte of chunk) {
            frequencies[byte]++;
        }
        const nonZeroFreqs = frequencies.filter(freq => freq > 0);
        if (nonZeroFreqs.length <= 1)
            return 1;
     ...
     */
    calculateChunkCorrelation(chunk1, chunk2) {
     ...
     */
    createPhaseContext(previousChunks = []) {
        return {
     ...
     */
    updatePhaseContext(context, chunk, phase) {
...
exports.QuantumPhaseAssigner = QuantumPhaseAssigner;
//# sourceMappingURL=QuantumPhaseAssigner.js.map