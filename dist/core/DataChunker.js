 ...
 */
class DataChunker {
    constructor(strategy = 'fixed-size', baseChunkSize = 4, minChunkSize = 1, maxChunkSize = 64) {
     ...
     */
    calculateEntropyScore(entropy, size) {
     ...
     */
    isPatternBoundary(data, position) {
     ...
     */
    isNaturalBoundary(data, position) {
        ...
        if (currentByte === 0 || prevByte === 0)
            return true;
        // Large value jump
        if (Math.abs(currentByte - prevByte) > 128)
            return true;
        // Repeated byte sequence end
        if (position >= 2) {
            const prev2Byte = data[position - 2];
            if (prevByte === prev2Byte && currentByte !== prevByte)
                return true;
        }
        return false;
    }
    ...
    optimizeStrategy(data) {
        ...
        const strategies = ['fixed-size', 'entropy-based', 'pattern-based', 'boundary-based'];
        let bestStrategy = 'fixed-size';
        let bestScore = 0;
     ...
     */
    validateParameters(baseChunkSize, minChunkSize, maxChunkSize) {
        if (minChunkSize < 1) {
...
//# sourceMappingURL=DataChunker.js.map