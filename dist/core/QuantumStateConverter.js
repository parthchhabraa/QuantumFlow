...
exports.QuantumStateConverter = void 0;
...
class QuantumStateConverter {
    constructor(quantumBitDepth = 8, chunkSize = 4) {
     ...
     */
    convertFromQuantumStates(states) {
        if (states.length === 0) {
     ...
     */
    convertChunkToQuantumState(chunk) {
        ...
        return new QuantumStateVector_1.QuantumStateVector(amplitudes, phase);
    }
    /**
     * Convert quantum state vector back to data chunk
     */
    convertQuantumStateToChunk(state) {
        const bytes = [];
    ...
    createQuantumAmplitudes(chunk) {
        ...
        while (amplitudes.length < Math.min(this._chunkSize * 2, maxAmplitudes)) {
            amplitudes.push(new Complex_1.Complex(0, 0));
        }
        // Normalize the amplitudes
        return QuantumMath_1.QuantumMath.normalizeAmplitudes(amplitudes);
    }
    /**
     * Apply Hadamard-like transformation to a byte value
     */
    applyHadamardTransformation(byte) {
        ...
        const baseAmplitude = Complex_1.Complex.fromPolar(magnitude, phase);
        ...
        for (let i = 0; i < bitsToProcess; i++) {
            ...
            for (const amp of currentAmplitudes) {
                if (bit === 0) {
                    ...
                    const [amp1, amp2] = QuantumMath_1.QuantumMath.hadamardTransform(amp);
                    newAmplitudes.push(amp1, amp2);
                }
                else {
                    // |1⟩ state: apply phase-shifted transformation
                    const phaseShift = Math.PI / 4; // 45-degree phase shift
                    const shiftedAmp = amp.multiply(Complex_1.Complex.fromPolar(1, phaseShift));
                    const [amp1, amp2] = QuantumMath_1.QuantumMath.hadamardTransform(shiftedAmp);
                    newAmplitudes.push(amp1, amp2);
                }
            }
     ...
     */
    extractByteFromAmplitude(amplitude, globalPhase) {
        ...
        const byteValue = Math.round((normalizedMagnitude * 128) + (normalizedPhase * 127)) % 256;
    ...
    analyzeDataPatterns(data) {
        ...
        analysis.entropy = QuantumMath_1.QuantumMath.calculateEntropy(probabilities);
     ...
     */
    calculatePatternComplexity(data) {
        if (data.length < 2)
            return 0;
        let complexity = 0;
     ...
     */
    optimizeForData(data) {
        const analysis = this.analyzeDataPatterns(data);
        return new QuantumStateConverter(analysis.recommendedBitDepth, analysis.recommendedChunkSize);
    }
    /**
     * Get conversion statistics
     */
    getConversionStats(originalSize, quantumStates) {
        const totalAmplitudes = quantumStates.reduce((sum, state) => sum + state.amplitudes.length, 0);
...
exports.QuantumStateConverter = QuantumStateConverter;
//# sourceMappingURL=QuantumStateConverter.js.map