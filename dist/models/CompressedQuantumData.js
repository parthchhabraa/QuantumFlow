...
exports.CompressedQuantumData = void 0;
const QuantumStateVector_1 = require("./QuantumStateVector");
const EntanglementPair_1 = require("./EntanglementPair");
 ...
 */
class CompressedQuantumData {
    constructor(quantumStates, entanglementMap, interferencePatterns, metadata, checksum) {
     ...
     */
    static create(quantumStates, entanglementPairs, interferencePatterns, originalSize, compressionConfig) {
        ...
        const compressed = new CompressedQuantumData(quantumStates, entanglementMap, interferencePatterns, metadata);
    ...
    static deserialize(buffer) {
        try {
            ...
            return new CompressedQuantumData(quantumStates, entanglementMap, data.interferencePatterns, data.metadata, data.checksum);
        }
        catch (error) {
     ...
     */
    clone() {
        return new CompressedQuantumData(this._quantumStates, this._entanglementMap, this._interferencePatterns, this._metadata);
    }
    /**
    ...
    calculateChecksum() {
        ...
        const stateHashes = this._quantumStates.map(state => {
            ...
            return QuantumMath_1.QuantumMath.quantumHash(bytes);
        });
        const entanglementHashes = Array.from(this._entanglementMap.values()).map(pair => {
            ...
            return QuantumMath_1.QuantumMath.quantumHash(combinedBytes);
        });
        const allHashes = [...stateHashes, ...entanglementHashes];
        const combinedHash = allHashes.join('');
        return QuantumMath_1.QuantumMath.quantumHash(Buffer.from(combinedHash));
    }
    /**
     * Serialize quantum states to JSON-compatible format
     */
    serializeQuantumStates() {
     ...
     */
    static deserializeQuantumStates(data) {
        return data.map(stateData => {
            const amplitudes = stateData.amplitudes.map((amp) => new (require('../math/Complex').Complex)(amp.real, amp.imaginary));
            const state = new QuantumStateVector_1.QuantumStateVector(amplitudes, stateData.phase);
     ...
     */
    static deserializeEntanglementMap(data) {
        const map = new Map();
        data.forEach(pairData => {
            ...
            const pair = new EntanglementPair_1.EntanglementPair(stateA, stateB, sharedInfo);
     ...
     */
    static deserializeQuantumState(data) {
        const amplitudes = data.amplitudes.map((amp) => new (require('../math/Complex').Complex)(amp.real, amp.imaginary));
        const state = new QuantumStateVector_1.QuantumStateVector(amplitudes, data.phase);
...
exports.CompressedQuantumData = CompressedQuantumData;
//# sourceMappingURL=CompressedQuantumData.js.map