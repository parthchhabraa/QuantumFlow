...
exports.MeetingAnalyticsEngine = void 0;
const events_1 = require("events");
class MeetingAnalyticsEngine extends events_1.EventEmitter {
    constructor() {
        super();
        this.currentMeeting = null;
        this.participantData = new Map();
        this.connectionStats = new Map();
        this.speakingTimes = new Map();
     ...
     */
    updateCompressionMetrics(originalSize, compressedSize, compressionTime, config) {
        if (!this.currentMeeting)
            return;
        ...
        compression.compressionEfficiency = Math.min(100, (compression.overallCompressionRatio * 50) +
     ...
     */
    updateNetworkMetrics(participantId, stats) {
        if (!this.currentMeeting)
            return;
     ...
     */
    updateQualityMetrics(participantId, qualityMetrics) {
        if (!this.currentMeeting)
            return;
        const participant = this.participantData.get(participantId);
        if (participant) {
     ...
     */
    updateSpeakingTime(currentSpeaker) {
        const now = Date.now();
     ...
     */
    addRecordingAnalytics(recordingSession) {
        if (!this.currentMeeting)
            return;
    ...
    calculateNetworkMetrics() {
        ...
        if (participantCount > 0) {
            ...
            this.currentMeeting.network.networkStabilityScore = Math.max(0, 100 - (this.currentMeeting.network.packetLossPercentage * 10) -
...
exports.MeetingAnalyticsEngine = MeetingAnalyticsEngine;
